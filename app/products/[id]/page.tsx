...
export default function ProductDetailPage({ params }: Props) {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div>
              ...
              <Link
                ...
                className="btn-accent w-full mt-5 text-base"
            ...
            <div>
              ...
              {/* Product name */}
              <h1 className="font-heading font-bold text-neutral-900 word-keep mb-4"
              ...
              {/* Description */}
              <p className="font-body text-base text-neutral-600 leading-korean word-keep mb-8">
                  ...
                  <ul className="space-y-2">
                    {product.features.map((feat) => (
                      <li key={feat} className="flex items-start gap-2.5 font-body text-sm text-neutral-600">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5C7A3E" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5" aria-hidden="true">
                          <path d="m9 12 2 2 4-4" />
                          <circle cx="12" cy="12" r="10" />
                        </svg>
                        {/* TODO: Replace with actual product features */}
                        {feat}
                      </li>
                    ))}
                  </ul>
                  ...
                  <tbody>
                    {product.specs.map(({ label, value }, i) => (
                      ...
                      >
                        ...
                        >
                          {/* TODO: Replace with actual spec labels */}
                          {label}
                        </td>
                        <td className="font-body text-sm text-neutral-700 px-5 py-3.5 bg-white">
                          {/* TODO: Replace with actual spec values */}
                          {value}
                        </td>
                      </tr>
                    ))}
              ...
              <p className="mt-3 font-body text-xs text-neutral-400 text-center sm:text-left">
                평일 09:00 – 18:00 · {PHONE}
              </p>
            </div>
          </div>
        </div>
      ...
      <section
        className="py-16"
        style={{ background: 'linear-gradient(135deg, #3A1C08 0%, #52280B 100%)' }}
      >
        <div className="container-brand text-center">
          <h2
            className="font-heading font-bold word-keep mb-3"
            style={{ color: '#FFFFFF', fontSize: 'clamp(1.5rem, 3vw, 2.25rem)' }}
          >
            {/* TODO: Replace with actual CTA heading */}
            구매 및 유통 문의
          </h2>
          <p className="font-body word-keep mb-7" style={{ color: 'rgba(251,248,243,0.65)' }}>
            {/* TODO: Replace with actual CTA description */}
            대량 구매, 유통 문의, 제품 상담은 전화 또는 온라인으로 문의해 주세요.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              <PhoneIcon color="#3A1C08" />
              전화 문의
            </a>
            <Link href="/contact" className="btn-secondary" style={{ borderColor: 'rgba(251,248,243,0.25)', color: 'rgba(251,248,243,0.82)' }}>
              온라인 문의하기 →
            </Link>
          </div>
        </div>
...
function CartIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#3A1C08" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
